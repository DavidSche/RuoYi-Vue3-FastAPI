...
class UserService:
    ...
    @classmethod
    async def check_user_name_unique_services(cls, query_db: AsyncSession, m_user: UserModel):
        """
        校验用户名是否唯一service
        ...
        :return: 校验结果
        """
        user_id = -1 if m_user.user_id is None else m_user.user_id
        user = await UserDao.get_user_by_info(query_db, UserModel(userName=m_user.user_name))
        if user and user.user_id != user_id:
            return CommonConstant.NOT_UNIQUE
        return CommonConstant.UNIQUE

    @classmethod
    async def check_phonenumber_unique_services(cls, query_db: AsyncSession, m_user: UserModel):
        """
        校验用户手机号是否唯一service
        ...
        """
        user_id = -1 if m_user.user_id is None else m_user.user_id
        user = await UserDao.get_user_by_info(query_db, UserModel(phonenumber=m_user.phonenumber))
        if user and user.user_id != user_id:
            return CommonConstant.NOT_UNIQUE
        return CommonConstant.UNIQUE

    @classmethod
    async def check_email_unique_services(cls, query_db: AsyncSession, m_user: UserModel):
        """
        校验用户邮箱是否唯一service
        ...
        """
        user_id = -1 if m_user.user_id is None else m_user.user_id
        user = await UserDao.get_user_by_info(query_db, UserModel(email=m_user.email))
        if user and user.user_id != user_id:
            return CommonConstant.NOT_UNIQUE
        return CommonConstant.UNIQUE

    @classmethod
    async def add_user_services(cls, query_db: AsyncSession, m_user: AddUserModel):
        """
        新增用户信息service
        ...
        :return: 新增用户校验结果
        """
        add_user = UserModel(**m_user.model_dump(by_alias=True))
        if not await cls.check_user_name_unique_services(query_db, m_user):
            raise ServiceException(message=f'新增用户{m_user.user_name}失败，登录账号已存在')
        elif m_user.phonenumber and not await cls.check_phonenumber_unique_services(query_db, m_user):
            raise ServiceException(message=f'新增用户{m_user.user_name}失败，手机号码已存在')
        elif m_user.email and not await cls.check_email_unique_services(query_db, m_user):
            raise ServiceException(message=f'新增用户{m_user.user_name}失败，邮箱账号已存在')
        else:
            try:
                add_result = await UserDao.add_user_dao(query_db, add_user)
                user_id = add_result.user_id
                if m_user.role_ids:
                    for role in m_user.role_ids:
                        await UserDao.add_user_role_dao(query_db, UserRoleModel(userId=user_id, roleId=role))
                if m_user.post_ids:
                    for post in m_user.post_ids:
    ...
    @classmethod
    async def edit_user_services(cls, query_db: AsyncSession, m_user: EditUserModel):
        """
        编辑用户信息service
        ...
        :return: 编辑用户校验结果
        """
        edit_user = m_user.model_dump(exclude_unset=True, exclude={'admin'})
        if m_user.type != 'status' and m_user.type != 'avatar' and m_user.type != 'pwd':
            del edit_user['role_ids']
            del edit_user['post_ids']
            del edit_user['role']
        if m_user.type == 'status' or m_user.type == 'avatar' or m_user.type == 'pwd':
            del edit_user['type']
        user_info = await cls.user_detail_services(query_db, edit_user.get('user_id'))
        if user_info.data and user_info.data.user_id:
            if m_user.type != 'status' and m_user.type != 'avatar' and m_user.type != 'pwd':
                if not await cls.check_user_name_unique_services(query_db, m_user):
                    raise ServiceException(message=f'修改用户{m_user.user_name}失败，登录账号已存在')
                elif m_user.phonenumber and not await cls.check_phonenumber_unique_services(query_db, m_user):
                    raise ServiceException(message=f'修改用户{m_user.user_name}失败，手机号码已存在')
                elif m_user.email and not await cls.check_email_unique_services(query_db, m_user):
                    raise ServiceException(message=f'修改用户{m_user.user_name}失败，邮箱账号已存在')
            try:
                await UserDao.edit_user_dao(query_db, edit_user)
                if m_user.type != 'status' and m_user.type != 'avatar' and m_user.type != 'pwd':
                    await UserDao.delete_user_role_dao(query_db, UserRoleModel(userId=m_user.user_id))
                    await UserDao.delete_user_post_dao(query_db, UserPostModel(userId=m_user.user_id))
                    if m_user.role_ids:
                        for role in m_user.role_ids:
                            await UserDao.add_user_role_dao(
                                query_db, UserRoleModel(userId=m_user.user_id, roleId=role)
                            )
                    if m_user.post_ids:
                        for post in m_user.post_ids:
                            await UserDao.add_user_post_dao(
                                query_db, UserPostModel(userId=m_user.user_id, postId=post)
                            )
                await query_db.commit()
    ...
    @classmethod
    async def delete_user_services(cls, query_db: AsyncSession, m_user: DeleteUserModel):
        """
        删除用户信息service
        ...
        """
        if m_user.user_ids:
            user_id_list = m_user.user_ids.split(',')
            try:
                for user_id in user_id_list:
                    user_id_dict = dict(
                        userId=user_id, updateBy=m_user.update_by, updateTime=m_user.update_time
                    )
    ...
    async def user_detail_services(cls, query_db: AsyncSession, user_id: Union[int, str]):
        ...
        if user_id != '':
            query_user = await UserDao.get_user_detail_by_id(query_db, int(user_id))
    ...
    @classmethod
    async def reset_user_services(cls, query_db: AsyncSession, m_user: ResetUserModel):
        """
        重置用户密码service
        ...
        :return: 重置用户校验结果
        """
        reset_user = m_user.model_dump(exclude_unset=True, exclude={'admin'})
        if m_user.old_password:
            user = (await UserDao.get_user_detail_by_id(query_db, user_id=m_user.user_id)).get('user_basic_info')
            if not PwdUtil.verify_password(m_user.old_password, user.password):
                raise ServiceException(message='修改密码失败，旧密码错误')
            elif PwdUtil.verify_password(m_user.password, user.password):
                raise ServiceException(message='新密码不能与旧密码相同')
            else:
                del reset_user['old_password']
        if m_user.sms_code and m_user.session_id:
            del reset_user['sms_code']
            del reset_user['session_id']
        try:
            reset_user['password'] = PwdUtil.get_password_hash(m_user.password)
    ...
    @classmethod
    async def get_user_role_allocated_list_services(cls, query_db: AsyncSession, m_user: UserRoleQueryModel):
        """
        根据用户id获取已分配角色列表

        :param query_db: orm对象
        :param m_user: 用户关联角色对象
        :return: 已分配角色列表
        """
        query_user = await UserDao.get_user_detail_by_id(query_db, m_user.user_id)
        post_ids = ','.join([str(row.post_id) for row in query_user.get('user_post_info')])
        role_ids = ','.join([str(row.role_id) for row in query_user.get('user_role_info')])
        user = UserInfoModel(
            **CamelCaseUtil.transform_result(query_user.get('user_basic_info')),
    ...
    @classmethod
    async def add_user_role_services(cls, query_db: AsyncSession, m_user: CrudUserRoleModel):
        """
        新增用户关联角色信息service

        :param query_db: orm对象
        :param m_user: 新增用户关联角色对象
        :return: 新增用户关联角色校验结果
        """
        if m_user.user_id and m_user.role_ids:
            role_id_list = m_user.role_ids.split(',')
            try:
                await UserDao.delete_user_role_by_user_and_role_dao(query_db, UserRoleModel(userId=m_user.user_id))
                for role_id in role_id_list:
                    await UserDao.add_user_role_dao(query_db, UserRoleModel(userId=m_user.user_id, roleId=role_id))
                await query_db.commit()
                return CrudResponseModel(is_success=True, message='分配成功')
            except Exception as e:
                await query_db.rollback()
                raise e
        elif m_user.user_id and not m_user.role_ids:
            try:
                await UserDao.delete_user_role_by_user_and_role_dao(query_db, UserRoleModel(userId=m_user.user_id))
                await query_db.commit()
                return CrudResponseModel(is_success=True, message='分配成功')
                ...
                await query_db.rollback()
                raise e
        elif m_user.user_ids and m_user.role_id:
            user_id_list = m_user.user_ids.split(',')
            try:
                for user_id in user_id_list:
                    user_role = await cls.detail_user_role_services(
                        query_db, UserRoleModel(userId=user_id, roleId=m_user.role_id)
                    )
                    if user_role:
                        continue
                    else:
    ...
    @classmethod
    async def delete_user_role_services(cls, query_db: AsyncSession, m_user: CrudUserRoleModel):
        """
        删除用户关联角色信息service

        :param query_db: orm对象
        :param m_user: 删除用户关联角色对象
        :return: 删除用户关联角色校验结果
        """
        if (m_user.user_id and m_user.role_id) or (m_user.user_ids and m_user.role_id):
            if m_user.user_id and m_user.role_id:
                try:
                    await UserDao.delete_user_role_by_user_and_role_dao(
                        query_db, UserRoleModel(userId=m_user.user_id, roleId=m_user.role_id)
                    )
                    await query_db.commit()
                    return CrudResponseModel(is_success=True, message='删除成功')
                except Exception as e:
    ...
    @classmethod
    async def detail_user_role_services(cls, query_db: AsyncSession, m_user: UserRoleModel):
        """
        获取用户关联角色详细信息service
        ...
        :return: 用户关联角色详细信息
        """
        user_role = await UserDao.get_user_role_detail(query_db, m_user)
